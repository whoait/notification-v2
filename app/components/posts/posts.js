...
const StatusNameField = ({record = {}}) => (
    <span>{record.status} </span>
);

    StatusNameField.defaultProps = {label: 'ステータス'};
...
export const PostEdit = (props) => (
    <Create {...props}>
        <SimpleForm redirect="show">
            <TextInput source="title" label="管理タイトル" validate={required}/>
            <CheckboxGroupInput source="bindversion" label="通知対象" choices={[
                {id: 'programming', name: 'クラウド'},
                {id: 'lifestyle', name: 'クライアント'},
                {id: 'photography', name: 'BiND10'},
            ...
            <TextInput source="ext_link" label="リンク URL"/>
